...
imcomm_select(int nfds, fd_set * readfds, fd_set * writefds, fd_set * exceptfds, struct timeval * timeout)
{
	...
	while (tmp != NULL) {
		...
		if (tmp->handle->use_ssl)
			FD_SET(SSL_get_fd(tmp->handle->ssl_handle), readfds);
		tmp = tmp->next;
	}
	...
	for (tmp = handles; tmp; tmp = tmp->next) {
		...
		if (FD_ISSET(tmp->handle->socket, readfds)) {
			...
			if (tmp->handle->use_ssl) {
				int ssl_ret = 0;
				do {
					ssl_ret = SSL_read(tmp->handle->ssl_handle,
						  tmp->handle->header + bytesread, 6 - bytesread);
					if (ssl_ret < 0) {
						int ssl_err = SSL_get_error(tmp->handle->ssl_handle, ssl_ret);
						printf("SSL error: %d %d\n", ssl_err, errno);
						SSL_shutdown(tmp->handle->ssl_handle);
						tmp->handle->socket = -1;
						tmp->handle->data = NULL;
				...
				ret = flap_decode(tmp->handle, tmp->handle->header,
						    tmp->handle->data);
				tmp->handle->data = NULL;
			} else {
				do {
			...
			if (tmp->handle->use_ssl) {
				if (tmp->handle->header_pos < 6) {
					if (SSL_read
					    (tmp->handle->ssl_handle,
					     &tmp->handle->header[tmp->handle->header_pos], 1) <= 0) {
						SSL_shutdown(tmp->handle->ssl_handle);
						tmp->handle->socket = -1;
						tmp->handle->connected = 0;
					...
					if (SSL_read
					    (tmp->handle->ssl_handle,
					&tmp->handle->data[tmp->handle->data_pos], 1) <= 0) {
